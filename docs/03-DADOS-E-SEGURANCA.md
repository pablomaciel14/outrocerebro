...
| `user_preferences` | área padrão ao entrar no workspace |
...
- cascata para destaques e marcadores de uma leitura removida (verificada em `tests/user-isolation.test.mjs`);
- o service worker do PWA (`public/sw.js`) não faz cache nem intercepta nenhuma resposta — existe só para instalabilidade, então não guarda PDFs nem dados privados fora do controle normal de `cache-control`.
